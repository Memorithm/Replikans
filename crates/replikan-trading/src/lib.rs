...
use execution_v2::{
    ApplyOutcomeV2, ExecutionEventKindV2, ExecutionEventV2, ExecutionOrderV2,
    ExecutionStatusV2, FillRecordV2, LifecycleBookV2,
};
use financial::{
...
impl From<rusqlite::Error> for Error {
    fn from(error: rusqlite::Error) -> Self { Self(error.to_string()) }
}
impl From<serde_json::Error> for Error {
    fn from(error: serde_json::Error) -> Self { Self(error.to_string()) }
}
impl From<financial::FinancialError> for Error {
    fn from(error: financial::FinancialError) -> Self { Self(error.to_string()) }
}
...
impl Config {
    fn validate(&self) -> Result<()> {
        if self.schema_version != 1 || self.venue != "paper"
            || self.account_id.trim().is_empty() || self.instruments.is_empty()
            || self.max_open_orders == 0 || self.max_intent_age_ms <= 0
        {
            return Err(Error("invalid configuration; only paper mode is enabled".into()));
        }
        ...
        if self.initial_balances.iter().any(|(asset, value)| asset.trim().is_empty() || value.is_negative()) {
...
pub trait VenueAdapter {
    ...
    fn submit(&mut self, intent: &Intent, config: &Config, now_ms: i64) -> Result<Vec<Observation>>;
...
enum Record {
    ...
    Observation { client_order_id: String, value: Observation },
}
...
fn balance_change(balances: &mut BTreeMap<String, SignedAmount>, asset: &str, delta: SignedAmount) -> Result<()> {
    let value = balances.get(asset).copied().unwrap_or(SignedAmount::ZERO).checked_add(delta)?;
...
impl State {
        ...
        match record {
            Record::Intent(intent) => {
                domain(self.book.register_intent(
                    intent.intent_id.clone(), intent.idempotency_key.clone(),
                    intent.agent_id.clone(), intent.client_order_id.clone(),
                    intent.request.clone(), intent.created_at_ms,
                ))?;
                self.intents.insert(intent.client_order_id.clone(), *intent.clone());
            }
            Record::Dispatch { client_order_id } => {
                let order = self.book.orders.get(client_order_id).ok_or_else(|| Error("unknown order".into()))?;
                if order.status != ExecutionStatusV2::PendingSubmit || !self.dispatched.insert(client_order_id.clone()) {
                    return Err(Error("submission already claimed or no longer pending".into()));
                }
            }
            Record::Observation { client_order_id, value } => {
                let receipt_identity = serde_json::to_string(&(client_order_id, value))?;
                if self.observations.contains(&receipt_identity) { return Ok(()); }
                let event = ExecutionEventV2 {
                    local_sequence: self.book.last_local_sequence.checked_add(1).ok_or_else(|| Error("sequence overflow".into()))?,
                ...
                if outcome != ApplyOutcomeV2::DuplicateFill {
                    if let ExecutionEventKindV2::Fill { price, quantity, fee_asset, fee_amount, .. } = &value.kind {
                        let intent = self.intents.get(client_order_id).ok_or_else(|| Error("missing intent".into()))?;
                        let rules = config.instruments.get(&intent.request.instrument_id).ok_or_else(|| Error("missing instrument".into()))?;
                        ...
                        balance_change(&mut self.balances, fee_asset, SignedAmount::ZERO.checked_sub(*fee_amount)?)?;
                    }
                }
            }
        }
    ...
    fn available(&self, config: &Config) -> Result<BTreeMap<String, SignedAmount>> {
        ...
        for (id, order) in &self.book.orders {
            if order.status.is_terminal() { continue; }
            let intent = self.intents.get(id).ok_or_else(|| Error("missing intent".into()))?;
            ...
            for (asset, amount) in requirements(intent, config)? {
                balance_change(&mut available, &asset, SignedAmount::ZERO.checked_sub(amount)?)?;
            }
        }
        Ok(available)
    }
...
fn requirements(intent: &Intent, config: &Config) -> Result<BTreeMap<String, SignedAmount>> {
    let rules = config.instruments.get(&intent.request.instrument_id).ok_or_else(|| Error("unsupported instrument".into()))?;
    ...
    if notional > config.max_order_notional { return Err(Error("order notional exceeds configured policy".into())); }
    let mut amounts = BTreeMap::new();
    match intent.request.side {
        Side::Buy => { amounts.insert(rules.quote_asset.clone(), SignedAmount::from(notional)); }
        Side::Sell => { amounts.insert(rules.base_asset.clone(), SignedAmount::from(intent.request.quantity)); }
    }
    balance_change(&mut amounts, &rules.quote_asset, SignedAmount::from(config.paper_quote_fee))?;
    Ok(amounts)
}

fn authorize(intent: &Intent, config: &Config, now_ms: i64) -> Result<()> {
    if [&intent.intent_id, &intent.idempotency_key, &intent.agent_id, &intent.client_order_id,
        &intent.decision_id, &intent.strategy_version, &intent.rationale].iter().any(|v| v.trim().is_empty())
        || intent.evidence_refs.is_empty() || intent.evidence_refs.iter().any(|v| v.trim().is_empty())
        || intent.created_at_ms > now_ms || intent.expires_at_ms < now_ms
        || intent.expires_at_ms < intent.created_at_ms
        || now_ms.checked_sub(intent.created_at_ms).is_none_or(|age| age > config.max_intent_age_ms)
        || intent.request.quantity > config.max_quantity
        || intent.request.reduce_only || intent.request.post_only
        || intent.request.tif != orders::TimeInForce::Gtc
    {
        return Err(Error("intent rejected by paper policy, validity or unsupported order flags".into()));
    }
    let rules = config.instruments.get(&intent.request.instrument_id).ok_or_else(|| Error("unsupported instrument".into()))?;
    intent.reference.validate_at(now_ms)?;
    if intent.reference.venue != config.venue || intent.reference.instrument_id != rules.instrument_id {
        return Err(Error("reference identity mismatch".into()));
    }
    validate_order(&intent.request, rules, Some(intent.reference.clone()), now_ms)?;
...
impl Runtime {
    pub fn open(path: impl AsRef<Path>, config: Config) -> Result<Self> {
        ...
        transaction.execute("INSERT OR IGNORE INTO trading_config (id,json) VALUES (1,?1)", [&json])?;
        let stored: String = transaction.query_row("SELECT json FROM trading_config WHERE id=1", [], |row| row.get(0))?;
        if stored != json { return Err(Error("configuration mismatch; implicit state migration refused".into())); }
    ...
    pub fn snapshot(&mut self) -> Result<Snapshot> {
        ...
        let recovery_required = state.book.orders.iter().filter(|(id, order)| {
            state.dispatched.contains(*id) && (order.status == ExecutionStatusV2::PendingSubmit || order.status.is_ambiguous()
                || matches!(order.status, ExecutionStatusV2::PendingCancel | ExecutionStatusV2::PendingAmend))
        }).map(|(id, _)| id.clone()).collect();
        Ok(Snapshot {
            schema_version: 1, mode: "paper", venue: self.config.venue.clone(), account_id: self.config.account_id.clone(),
            journal_sequence: state.sequence, journal_hash: state.hash,
            balances: state.balances, orders: state.book.orders.into_values().collect(),
            fills: state.book.fills.into_values().collect(), recovery_required,
        })
    }
    ...
    pub fn prepare(&mut self, intent: Intent, now_ms: i64) -> Result<()> {
        ...
        if let Some(existing) = state.intents.get(&intent.client_order_id) {
            if existing == &intent { return Ok(()); }
            return Err(Error("client order identity reused with changed content".into()));
        }
        ...
        if state.book.orders.values().filter(|order| !order.status.is_terminal()).count() >= self.config.max_open_orders {
        ...
        append(&transaction, &mut state, &self.config, Record::Intent(Box::new(intent)))?;
        transaction.commit()?;
        Ok(())
    }
    ...
    pub fn dispatch(&mut self, id: &str, adapter: &mut dyn VenueAdapter, now_ms: i64) -> Result<()> {
        self.check_adapter(adapter)?;
        let transaction = self.connection.transaction_with_behavior(TransactionBehavior::Immediate)?;
        let mut state = replay(&transaction, &self.config)?;
        let intent = state.intents.get(id).cloned().ok_or_else(|| Error("unknown intent".into()))?;
        authorize(&intent, &self.config, now_ms)?;
        if state.available(&self.config)?.values().any(|balance| balance.is_negative()) {
            return Err(Error("unresolved balance or reservation deficit".into()));
        }
        append(&transaction, &mut state, &self.config, Record::Dispatch { client_order_id: id.to_owned() })?;
        transaction.commit()?;
        match adapter.submit(&intent, &self.config, now_ms) {
            Ok(observations) => self.record_observations(id, observations),
            Err(error) => {
                self.record_observations(id, vec![Observation { native_sequence: None, received_at_ms: now_ms,
                    kind: ExecutionEventKindV2::SubmitUnknown { reason: "adapter did not return a definitive outcome".into() } }])?;
                Err(error)
            }
        }
    }

    pub fn reconcile(&mut self, id: &str, adapter: &mut dyn VenueAdapter, now_ms: i64) -> Result<()> {
        self.check_adapter(adapter)?;
        let observations = adapter.query(id, now_ms)?.ok_or_else(|| Error("order not found; absence is not permission to resubmit".into()))?;
    ...
    pub fn cancel(&mut self, id: &str, adapter: &mut dyn VenueAdapter, now_ms: i64) -> Result<()> {
        ...
        match adapter.cancel(id, now_ms) {
            ...
            Err(error) => {
                self.record_observations(id, vec![Observation { native_sequence: None, received_at_ms: now_ms,
                    kind: ExecutionEventKindV2::CancelUnknown { reason: "adapter did not confirm cancellation".into() } }])?;
                Err(error)
            }
        }
    }
    ...
    pub fn record_observations(&mut self, id: &str, observations: Vec<Observation>) -> Result<()> {
        if observations.is_empty() { return Err(Error("empty adapter receipt".into())); }
        let transaction = self.connection.transaction_with_behavior(TransactionBehavior::Immediate)?;
        let mut state = replay(&transaction, &self.config)?;
        if !state.dispatched.contains(id) { return Err(Error("receipt for an undispatched intent".into())); }
        for value in observations {
            append(&transaction, &mut state, &self.config, Record::Observation { client_order_id: id.to_owned(), value })?;
        }
        transaction.commit()?;
        Ok(())
    }
    ...
    pub fn export(&mut self) -> Result<serde_json::Value> {
        let transaction = self.connection.transaction()?;
        let state = replay(&transaction, &self.config)?;
        let mut query = transaction.prepare("SELECT sequence,payload,hash FROM trading_journal ORDER BY sequence")?;
        ...
        Ok(serde_json::json!({"schema_version":1,"config":self.config,"journal_hash":state.hash,"entries":entries}))
    }
}

fn replay(connection: &Connection, config: &Config) -> Result<State> {
    let mut state = State::empty(config)?;
    let mut query = connection.prepare("SELECT sequence,payload,hash FROM trading_journal ORDER BY sequence")?;
    let rows = query.query_map([], |row| Ok((row.get::<_, i64>(0)?, row.get::<_, String>(1)?, row.get::<_, String>(2)?)))?;
    for row in rows {
        let (sequence, payload, hash) = row?;
        if sequence != state.sequence.checked_add(1).ok_or_else(|| Error("sequence overflow".into()))?
            || sequence > 1_000_000 || payload.len() > 1_048_576
...
fn append(connection: &Connection, state: &mut State, config: &Config, record: Record) -> Result<()> {
    let payload = serde_json::to_string(&record)?;
    let sequence = state.sequence.checked_add(1).ok_or_else(|| Error("sequence overflow".into()))?;
    if payload.len() > 1_048_576 || sequence > 1_000_000 { return Err(Error("journal budget exceeded".into())); }
    let mut candidate = state.clone();
    candidate.apply(&record, config)?;
    let hash = digest(&state.hash, sequence, &payload);
    connection.execute("INSERT INTO trading_journal(sequence,payload,hash) VALUES (?1,?2,?3)", params![sequence, payload, hash])?;
