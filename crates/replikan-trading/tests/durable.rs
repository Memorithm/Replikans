use replikan_trading::{
    Config, Error, Intent, Observation, Result, Runtime, VenueAdapter, paper::PaperVenue,
    execution_v2::{ExecutionEventKindV2, ExecutionStatusV2, FillKey},
    financial::{ExactInstrumentRules, ExactOrderRequest, ExactOrderType, NonNegativeMoney, Price, Quantity, ReferencePrice, SignedAmount},
...
fn config() -> Result<Config> {
    let rules = ExactInstrumentRules {
        venue: "paper".into(), instrument_id: "TEST-QUOTE".into(), base_asset: "TEST".into(),
        quote_asset: "QUOTE".into(), rules_version: "test-v1".into(), price_tick: Price::parse("0.01")?,
        quantity_step: Quantity::parse("0.01")?, min_quantity: Quantity::parse("0.01")?,
        max_quantity: None, min_notional: NonNegativeMoney::parse("1")?, max_notional: None,
    };
    Ok(Config {
        schema_version: 1, venue: "paper".into(), account_id: "test-account".into(),
        instruments: BTreeMap::from([("TEST-QUOTE".into(), rules)]),
        initial_balances: BTreeMap::from([("QUOTE".into(), SignedAmount::parse("1000")?)]),
        max_quantity: Quantity::parse("10")?, max_order_notional: NonNegativeMoney::parse("1000")?,
        max_open_orders: 5, max_intent_age_ms: 1000, paper_quote_fee: NonNegativeMoney::parse("1")?,
    })
}

fn intent(id: &str, side: Side, price: &str) -> Result<Intent> {
    Ok(Intent {
        intent_id: format!("intent-{id}"), idempotency_key: format!("key-{id}"), agent_id: "test-agent".into(),
        client_order_id: id.into(), decision_id: format!("decision-{id}"), strategy_version: "fixture-v1".into(),
        evidence_refs: vec!["fixture:reference".into()], rationale: "deterministic test decision".into(),
        created_at_ms: 1000, expires_at_ms: 2000,
        request: ExactOrderRequest {
            instrument_id: "TEST-QUOTE".into(), side, order_type: ExactOrderType::Market,
            quantity: Quantity::parse("1")?, tif: TimeInForce::Gtc, reduce_only: false, post_only: false,
            rules_version: "test-v1".into(),
        },
        reference: ReferencePrice { venue: "paper".into(), instrument_id: "TEST-QUOTE".into(),
            price: Price::parse(price)?, observed_at_ms: 1000, valid_until_ms: 2000 },
    })
}

fn open(directory: &Path) -> Result<(Runtime, PaperVenue)> {
    Ok((Runtime::open(directory.join("runtime.sqlite"), config()?)?,
...
fn duplicate_intent_is_idempotent_but_duplicate_dispatch_cannot_send() -> TestResult {
    ...
    assert_eq!(runtime.snapshot()?.balances["QUOTE"].as_decimal_string(), "899");
...
impl VenueAdapter for LoseReply {
    fn identity(&self) -> (&str, &str) { self.0.identity() }
    ...
    fn query(&mut self, id: &str, now: i64) -> Result<Option<Vec<Observation>>> { self.0.query(id, now) }
    fn cancel(&mut self, id: &str, now: i64) -> Result<Vec<Observation>> { self.0.cancel(id, now) }
}
...
fn lost_reply_is_unknown_then_reconciled_without_resubmit() -> TestResult {
    ...
    runtime.prepare(intent("buy", Side::Buy, "100")?, 1000)?;
    assert!(runtime.dispatch("buy", &mut venue, 1000).is_err());
    assert_eq!(runtime.snapshot()?.orders[0].status, ExecutionStatusV2::SubmitUnknown);
    drop(runtime);
    ...
    assert_eq!(reopened.snapshot()?.orders[0].status, ExecutionStatusV2::Filled);
    assert_eq!(reopened.snapshot()?.balances["QUOTE"].as_decimal_string(), "899");
    Ok(())
}

struct ExitAdapter { venue: PaperVenue, before_send: bool }
impl VenueAdapter for ExitAdapter {
    fn identity(&self) -> (&str, &str) { self.venue.identity() }
    fn submit(&mut self, intent: &Intent, config: &Config, now: i64) -> Result<Vec<Observation>> {
        if !self.before_send { self.venue.submit(intent, config, now)?; }
        std::process::exit(86)
    }
    fn query(&mut self, id: &str, now: i64) -> Result<Option<Vec<Observation>>> { self.venue.query(id, now) }
    fn cancel(&mut self, id: &str, now: i64) -> Result<Vec<Observation>> { self.venue.cancel(id, now) }
}

#[test]
fn crash_child() -> TestResult {
    let Ok(path) = std::env::var("REPLIKAN_TEST_CRASH_DIRECTORY") else { return Ok(()); };
    let (mut runtime, venue) = open(Path::new(&path))?;
    runtime.prepare(intent("crash", Side::Buy, "100")?, 1000)?;
    runtime.dispatch("crash", &mut ExitAdapter { venue, before_send: std::env::var_os("REPLIKAN_TEST_BEFORE_SEND").is_some() }, 1000)?;
...
fn real_process_death_after_claim_and_after_external_commit_is_recoverable() -> TestResult {
    for before_send in [false, true] {
        ...
        child.args(["--exact", "crash_child", "--nocapture"])
            .env("REPLIKAN_TEST_CRASH_DIRECTORY", directory.path());
        if before_send { child.env("REPLIKAN_TEST_BEFORE_SEND", "1"); }
...
fn database_failure_rolls_back_receipt_batch_then_query_recovers() -> TestResult {
    ...
    assert!(runtime.dispatch("buy", &mut venue, 1000).is_err());
    assert_eq!(runtime.snapshot()?.orders[0].status, ExecutionStatusV2::PendingSubmit);
    assert_eq!(runtime.snapshot()?.balances["QUOTE"].as_decimal_string(), "1000");
...
fn reservations_expiry_and_sell_inventory_are_enforced() -> TestResult {
    let directory = TempDir::new()?;
    let (mut runtime, mut venue) = open(directory.path())?;
    assert!(runtime.prepare(intent("sell", Side::Sell, "100")?, 1000).is_err());
    let mut first = intent("first", Side::Buy, "100")?;
    first.request.quantity = Quantity::parse("9")?;
    runtime.prepare(first, 1000)?;
    assert!(runtime.prepare(intent("second", Side::Buy, "100")?, 1000).is_err());
    assert!(runtime.dispatch("first", &mut venue, 2001).is_err());
    assert!(runtime.prepare(intent("stale", Side::Buy, "100")?, 999).is_err());
    Ok(())
}
...
fn cancel_open_limit_is_durable_and_query_is_idempotent() -> TestResult {
    ...
    order.request.order_type = ExactOrderType::Limit { price: Price::parse("90")? };
    runtime.prepare(order, 1000)?;
    runtime.dispatch("limit", &mut venue, 1000)?;
    assert_eq!(runtime.snapshot()?.orders[0].status, ExecutionStatusV2::Open);
    runtime.cancel("limit", &mut venue, 1001)?;
    runtime.reconcile("limit", &mut venue, 1002)?;
    assert_eq!(runtime.snapshot()?.orders[0].status, ExecutionStatusV2::Canceled);
    assert_eq!(runtime.snapshot()?.balances["QUOTE"].as_decimal_string(), "1000");
    Ok(())
}
...
fn partial_fills_deduplicate_across_transport_sequences_and_charge_third_asset_fee() -> TestResult {
    ...
    runtime.prepare(order, 1000)?;
    runtime.dispatch("limit", &mut venue, 1000)?;
    let mut receipt = Observation { native_sequence: Some(1), received_at_ms: 1001,
        kind: ExecutionEventKindV2::Fill {
            key: FillKey { venue: "paper".into(), account_id: "test-account".into(), instrument_id: "TEST-QUOTE".into(), trade_id: "fixture-fill".into() },
            occurred_at_ms: 1001, price: Price::parse("90")?, quantity: Quantity::parse("0.2")?,
            fee_asset: "FEE".into(), fee_amount: SignedAmount::parse("0.01")?,
        } };
...
fn journal_tampering_and_configuration_drift_fail_closed() -> TestResult {
    ...
    let connection = rusqlite::Connection::open(directory.path().join("runtime.sqlite"))?;
    connection.execute("UPDATE trading_journal SET hash='altered' WHERE sequence=1", [])?;
