...
| Basse | Preuves textuelles | Journal de décisions persistable, mais toujours sans sceau cryptographique. |
...
- Journal `REPLIKANS_DECISION_V1` : encode/decode fail-closed, persistance append-only.

## Recommandations suivantes

1. Vérification cryptographique des preuves de ledger (mentionnée dans SECURITY).
2. Matrice d’adaptateurs (réseau / exchange / mining) avec dry-run obligatoire.
3. Property tests plus larges sur timestamps, preuves vides et fenêtres de fitness.
4. Ne pas activer d’adaptateur live tant que le paper runtime n’a pas d’évidence bornée dans le temps.
